...
There are a number of fields that act as codes and that are used as primary and secondary keys to link loader tables together. Codes that begin with `user_` are supplied by the data contributor. Codes that begin with `vb_` are created by the database upon data upload, and where reference refer to codes that already exist in, and are retrievable via `vb_get` functions. For more details on downloading data from VegBank, see the [download vignette](https://nceas.github.io/vegbankr/articles/download.html).

### Diagram

Below is an Entity-Relationship diagram describing the relationship between the loader tables described above. Note that not all fields are shown for each table - the focus on this diagram is showing keys and relationships between tables. Some representative fields for each table are included, however. Click the diagram to expand in a new tab.

<a href="images/loader-tables-diagram.svg" target="_blank">
![Click to Zoom](images/loader-tables-diagram.svg)
</a>
...
This loader table contains data about the observed plants, cover, and strata in a given plot. VegBank separates plant records into three components: observations, interpretations, and importance. A taxon observation (`user_to_code`) represents seeing a plant at a specific place and time. The taxon importance (`user_tm_code`) indicates unique observations of measurable data like percent cover and stem counts. Because a single plant can occur in multiple vegetation layers, one observation of a plant on a plot (`user_to_code`) may have several importance (`user_tm_code`) records, each documenting the plant's characteristics in a different stratum. Therefore `user_tm_code` is a key that is unique for each combination of `user_ob_code`, plant name, and strata in this table. `user_to_code` is a key that is unique for each combination of `user_ob_code` and plant name.

The `user_ob_code` is a required foreign key that links the plant to a plot observation. All values in this field must be present in the plot observations loader table. `user_sr_code` is a foreign key that corresponds to the `strata` loader table, described below.
...
Each strata name (shrub, tree) must also have a VegBank strata method associated with it. The methods give the allowed strata values and descriptions of each strata within a method. A strata name within a strata method is represented by a unique `vb_sy_code`. To see available codes, use the code snippet below the table. The strata method is linked to an observation via the required `user_ob_code` field. `user_sr_code` is a required identifier that provides a key to each unique strata observation in the strata cover table.
...
Taxon interpretations associate the plants in the Strata Cover table with an existing VegBank plant concept code. To get a list of existing plant concepts, use the `vb_get_plant_concepts` function. The taxon interpretation (`user_ti_code`) identifier, and this table as a whole, indicates what species the observer believes a plant observation (`user_to_code`, from strata cover) to be. Separating interpretations from observations allows multiple researchers to assign different interpretations to the same observation if they disagree on identification.  Note that a person with a role is also required for this table, so one of `user_py_code` (present in the `parties` loader table) or `vb_py_code` (an existing VegBank party) must exist, along with their role, in `vb_ar_code`.
...
This table supports detailed plant demographic data collection, including
...
To do a test upload, point to the test instance. This is a development sandbox version of the production VegBank. Because it is a sandbox, some data in this instance may be repeated or erroneous, and persistence of both the instance and the data it holds is not guaranteed. 

```{r, eval = FALSE}
vb_set_base_url("https://api-dev.vegbank.org")
```

To upload to production VegBank, use the following URL:

```{r, eval = FALSE}
vb_set_base_url("https://api.vegbank.org")
```

Next, get a token by logging into [http://api-dev.vegbank.org/login](http://api-dev.vegbank.org/login) for the sandbox environment, or [http://api.vegbank.org/login](http://api.vegbank.org/login) for production. After logging in, you should see a JSON document that contains an access token and refresh token. The easiest way to get this information into R is to select the "Raw Data" option in your browser (if available) to get the plain text JSON. Copy this to your clipboard, and paste it into R to save to the variable `token`. You'll need to encase this string in **single quotes**. Double quotes will give a syntax error.
...
Once you get a successful dry run, set `dry_run` to `FALSE` to complete your upload. Uploaded data will be visible at [https://dev.vegbank.org/beta/](https://dev.vegbank.org/beta/) for the sandbox instance, and [https://vegbank.org/](https://vegbank.org/) for production.
